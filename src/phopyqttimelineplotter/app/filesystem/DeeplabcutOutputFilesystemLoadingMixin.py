    ...
    def load_data(self):
        if self.file_format == DeeplabCutOutputFileType.CSV:
            ...
            self.data = pd.read_excel(
                self.get_file_path(), "Sheet1", index_col=None, na_values=["NA"]
            )
...
class DeeplabcutFilesystemLoader(QObject):
    ...
    def __init__(self, deeplabcutFilePaths, parent=None):
        super(DeeplabcutFilesystemLoader, self).__init__(
            parent=parent
    ...
    def add_deeplabcut_file_path(self, newDeeplabcutFilePath):
        if newDeeplabcutFilePath in self.deeplabcutFilePaths:
            print(
                "WARNING: {0} is already in deeplabcutFilePaths! Not adding again.".format(
                    str(newDeeplabcutFilePath)
                )
            )
            return False
        ...
        if newDeeplabcutFilePath in self.loadedDeeplabcutFiles:
            print(
                "WARNING: {0} is already in loadedDeeplabcutFiles! It's already been processed, so we're not adding it again.".format(
                    str(newDeeplabcutFilePath)
                )
            )
            return False
    ...
    def loadDeeplabcutFiles(
        ...
        if usePhoServerFormat:
            ...
            ) = DeeplabcutEventsLoader.loadDeeplabcutDataFromPhoServerFormat(
                deeplabcutFilePath, shouldUseStdOutFormat=phoServerFormatIsStdOut
            )
        else:
            (
                dateNums,
                dateTimes,
                onesEventFormatDataArray,
            ) = DeeplabcutEventsLoader.loadDeeplabcutDataFromMatlabFormat(
                deeplabcutFilePath
            )
        ...
        if limitedVariablesToCreateEventsFor is not None:
            ...
            active_deeplabcut_variable_names = (
                DeeplabcutEventsLoader.deeplabcut_variable_names
            )
        ...
        for variableIndex in range(0, numVariables):
            ...
            dataArrayVariableIndex = (
                DeeplabcutEventsLoader.deeplabcut_variable_indicies_dict[
                    currVariableName
                ]
            )
            currVariableDataValues = onesEventFormatDataArray[:, dataArrayVariableIndex]
            currVariableColorTuple = mcolors.to_rgb(
                DeeplabcutEventsLoader.deeplabcut_variable_colors_dict[currVariableName]
            )
            ...
            for index, anActiveTimestamp in enumerate(activeTimestamps):
                ...
                if shouldCreateEvent:
                    currExtendedInfoDict = {
                        ...
                        "event_type": DeeplabcutEventsLoader.deeplabcut_variable_event_type[
                            dataArrayVariableIndex
                        ],
                        "dispense_type": DeeplabcutEventsLoader.deeplabcut_variable_event_type[
                            dataArrayVariableIndex
                        ],
                        "port": DeeplabcutEventsLoader.deeplabcut_variable_port_location[
                            dataArrayVariableIndex
                        ],
                    }
                    currRecord = FilesystemDeeplabcutEvent_Record(
            ...
            variableData.append(
                {
                    ...
                    "variableSpecificEvents": deeplabcutVariableSpecificEvents,
                }
            )
        ...
        try:
            ...
            keyfun = lambda x: x.startTime  # use a lambda if no operator module
        else:
        ...
        for index, anActiveEvent in enumerate(deeplabcutEvents):
            ...
            if anActiveEvent.extended_data["event_type"] == "BeamBreak":
                ...
                previousFoundBeambreakEventTimestamp[
                    anActiveEvent.extended_data["port"]
                ...
                if currIsInvalidEvent:
                    ...
                    allInvalidDispenseEventTimestamps.append(
                        currFoundDispenseEventTimestamp
                    )
    ...
    @staticmethod
    def loadDeeplabcutEventsFile_loadFromFile(
        deeplabcutFilePath, usePhoServerFormat=False, phoServerFormatIsStdOut=True
    ):
        ## Load the Deeplabcut events data from an exported MATLAB file
        # If shouldLimitEventsToVideoDates is True then only events that fall between the earliest video start date and the latest video finish date are included
        # If shouldLimitEventsToVariables is not None, then only events that are of type of the variable with the name in the array are included
    ...
    def loadDeeplabcutEventsFile(
        ...
        (
            dateTimes,
            onesEventFormatDataArray,
        ) = DeeplabcutFilesystemLoader.loadDeeplabcutEventsFile_loadFromFile(
            deeplabcutFilePath, usePhoServerFormat, phoServerFormatIsStdOut
        )
            ...
            variableData.append(
                {
                    ...
                    "values": activeValues,
                    "videoIndicies": activeVideoIndicies,
                    "variableSpecificRecords": deeplabcutVariableSpecificRecords,
                }
            )
    ...
    def load_deeplabcut_data_files(self, deeplabcutDataFilePaths):
        print(
            "DeeplabcutFilesystemLoader.load_deeplabcut_data_files(deeplabcutDataFilePaths: {0})".format(
                str(deeplabcutDataFilePaths)
            )
        )
        # Pass the function to execute
        self.deeplabcutFilesystemWorker = VideoMetadataWorker(
            deeplabcutDataFilePaths, self.on_load_deeplabcut_data_files_execute_thread
        )  # Any other args, kwargs are passed to the run function
        self.deeplabcutFilesystemWorker.signals.result.connect(
            self.on_load_deeplabcut_data_files_print_output
        )
        self.deeplabcutFilesystemWorker.signals.finished.connect(
            self.on_load_deeplabcut_data_files_thread_complete
        )
        self.deeplabcutFilesystemWorker.signals.progress.connect(
            self.on_load_deeplabcut_data_files_progress_fn
        )
    ...
    # Threads:
    @pyqtSlot(list, int)
    def on_load_deeplabcut_data_files_progress_fn(
        self, active_deeplabcut_data_file_paths, n
    ):
        self.pending_operation_status.update(n)
        self.deeplabcutDataFileLoaded.emit()
        print("%d%% done" % n)

    def on_load_deeplabcut_data_files_execute_thread(
        self, active_deeplabcut_data_file_paths, progress_callback
    ):
        currProgress = 0.0
        parsedFiles = 0
        numPendingFiles = len(active_deeplabcut_data_file_paths)
        self.pending_operation_status.restart(
            OperationTypes.FilesystemDeeplabcutFileLoad, numPendingFiles
        )

        for (sub_index, aFoundDeeplabcutDataFile) in enumerate(
            active_deeplabcut_data_file_paths
        ):
            ...
            (
                dateTimes,
                deeplabcutEventContainers,
            ) = DeeplabcutFilesystemLoader.loadDeeplabcutEventsFile(
                aFoundDeeplabcutDataFile,
                self.videoStartDates,
                self.videoEndDates,
            ...
            outEventFileObj.set_loaded_values(
                dateTimes, [], [], deeplabcutEventContainers
            )
            ...
            if not (aFoundDeeplabcutDataFile in self.cache.keys()):
                ...
                print(
                    "WARNING: data path {0} already exists in the cache. Updating its DLC Data objects list...".format(
                        str(aFoundDeeplabcutDataFile)
                    )
                )
                self.cache[aFoundDeeplabcutDataFile] = outEventFileObj
                pass
            ...
            progress_callback.emit(
                active_deeplabcut_data_file_paths, (parsedFiles * 100 / numPendingFiles)
            )
    ...
    @pyqtSlot(list)
    def on_load_deeplabcut_data_files_thread_complete(
        self, finished_loaded_deeplabcut_data_files
    ):
        print(
            "THREAD on_load_deeplabcut_data_files_thread_complete(...)! {0}".format(
                str(finished_loaded_deeplabcut_data_files)
            )
        )
