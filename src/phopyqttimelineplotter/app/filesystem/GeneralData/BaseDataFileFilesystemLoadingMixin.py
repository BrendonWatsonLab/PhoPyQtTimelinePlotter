...
class BaseDataEventFile(QObject):
    ...
    def set_loaded_values(
        self, dateTimes, variableData, dataEventsContainerArray, phoServerFormatArgs
    ):
...
class BaseDataFilesystemLoader(QObject):
    ...
    def __init__(self, dataFilePaths, parent=None):
        super(BaseDataFilesystemLoader, self).__init__(
            parent=parent
    ...
    def add_file_path(self, newBaseDataFilePath):
        if newBaseDataFilePath in self.dataFilePaths:
            print(
                "WARNING: {0} is already in dataFilePaths! Not adding again.".format(
                    str(newBaseDataFilePath)
                )
            )
            return False
        ...
        if newBaseDataFilePath in self.loadedDataFiles:
            print(
                "WARNING: {0} is already in loadedBaseDataFiles! It's already been processed, so we're not adding it again.".format(
                    str(newBaseDataFilePath)
                )
            )
            return False
    ...
    def load_data_files(self, dataFilePaths):
        print(
            "BaseDataFilesystemLoader.load_data_files(dataFilePaths: {0})".format(
                str(dataFilePaths)
            )
        )
        # Pass the function to execute
        self.dataFilesystemWorker = FileMetadataWorker(
            dataFilePaths, self.on_load_data_files_execute_thread
        )  # Any other args, kwargs are passed to the run function
        self.dataFilesystemWorker.signals.result.connect(
            self.on_load_data_files_print_output
        )
        self.dataFilesystemWorker.signals.finished.connect(
            self.on_load_data_files_thread_complete
        )
        self.dataFilesystemWorker.signals.progress.connect(
            self.on_load_data_files_progress_fn
        )
    ...
    def on_load_data_files_progress_fn(self, latest_file_result_list, n):
        ...
        outEventFileObj = latest_file_result_list[
            1
        ]  # outEventFileObj: BaseDataEventFile type object
        print(
            "on_load_data_files_progress_fn(..., n: {}): file: {}".format(
                str(n), str(aFoundBaseDataDataFile)
            )
        )
        ...
        if not (aFoundBaseDataDataFile in self.cache.keys()):
            print(
                "Creating new cache entry for {}...".format(str(aFoundBaseDataDataFile))
            )
            ...
            print(
                "WARNING: data file path {} already exists in the cache. Updating its values...".format(
                    str(aFoundBaseDataDataFile)
                )
            )
            self.cache[aFoundBaseDataDataFile] = outEventFileObj
            pass
    ...
    def on_load_data_files_execute_thread(
        ...
        self.pending_operation_status.restart(
            OperationTypes.FilesystemBaseDataFileLoad, numPendingFiles
        )
        ...
        for (sub_index, aFoundBaseDataDataFile) in enumerate(active_data_file_paths):
            ...
            (
                ...
                dataEventContainers,
                phoServerFormatArgs,
            ) = BaseDataFilesystemLoader.loadBaseDataEventsFile(
                aFoundBaseDataDataFile,
                self.videoStartDates,
                self.videoEndDates,
            ...
            if not (aFoundBaseDataDataFile in active_cache.keys()):
                ...
                # Parent already exists
                print(
                    "WARNING: data file path {} already exists in the temporary cache. Updating its values...".format(
                        str(aFoundBaseDataDataFile)
                    )
                )
                active_cache[aFoundBaseDataDataFile] = outEventFileObj
            ...
            progress_callback.emit(
                [aFoundBaseDataDataFile, outEventFileObj],
    ...
    def on_load_data_files_thread_complete(self, finished_loaded_data_files):
        print(
            "THREAD on_load_data_files_thread_complete(...)! {0}".format(
                str(finished_loaded_data_files)
            )
        )
