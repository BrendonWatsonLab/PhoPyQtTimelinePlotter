...
class VideoSpecificThumbnailCache(QObject):
    ...
    frame_thumbnails_updated = pyqtSignal(
        str, VideoThumbnail
    )  # (cache_video_file_name: str, frameThumbnailResult: VideoThumbnail)

    def __init__(self, videoFile, parent=None):
        super().__init__(parent=parent)
        self._videoFile = videoFile
        self._frameResultsDict = (
            dict()
        )  # a dict(int:VideoThumbnail) for a given video file.
    ...
    def update_frame_thumbnail_results(self, updatedThumbnailResultsList):
        for aThumbnailResult in updatedThumbnailResultsList:
            ...
            # Emit the signal
            self.frame_thumbnails_updated.emit(
                self.get_video_file_name(), aThumbnailResult
            )
...
class VideoPreviewThumbnailGenerator(QObject):
    ...
    videoThumbnailGenerationComplete = pyqtSignal(
        str, list
    ...
    def __init__(self, videoFilePaths, thumbnailSizes=[160, 80, 40], parent=None):
        super(VideoPreviewThumbnailGenerator, self).__init__(
            parent=parent
        )  # Call the inherited classes __init__ method
        self.cache = (
            dict()
        )  # a [str:VideoSpecificThumbnailCache] dict that holds the VideoSpecificThumbnailCache object for each video file
    ...
    def reload_data(self, restricted_video_file_paths, desired_frame_indicies):
        ...
        for aPath in restricted_video_file_paths:
            if (aPath in self.loadedVideoFiles) or (
                aPath in self.pendingVideoFilePaths
            ):
                # self.pendingVideoFilePaths.append(aPath) # Add the path to the pending paths again.
                final_paths.append(aPath)
            else:
                print(
                    "Warning: {0} isn't in self.videoFilePath. Add it using add_video_path(...) before calling reload_data(...)".format(
                        str(aPath)
                    )
                )
                continue

        self.generate_video_thumbnails(
            final_paths, desired_frame_indicies, self.desiredThumbnailSizes
        )
    ...
    def add_video_path(self, newVideoFilePath):
        if newVideoFilePath in self.pendingVideoFilePaths:
            print(
                "WARNING: {0} is already in videoFilePaths! Not adding again.".format(
                    str(newVideoFilePath)
                )
            )
            return False
        ...
        if newVideoFilePath in self.loadedVideoFiles:
            print(
                "WARNING: {0} is already in loadedVideoFiles! It's already been processed, so we're not adding it again.".format(
                    str(newVideoFilePath)
                )
            )
            return False
    ...
    def reload_on_video_paths_changed(self):
        ...
        if len(self.pendingVideoFilePaths) > 0:
            self.generate_video_thumbnails(
                self.pendingVideoFilePaths,
                self.desiredVideoFrames,
                self.desiredThumbnailSizes,
            )
    ...
    def generate_video_thumbnails(
        self, videoPaths, desired_frame_indicies, desired_thumbnail_sizes
    ):
        print(
            "VideoPreviewThumbnailGenerator.generate_video_thumbnails(videoPaths: {0})".format(
                str(videoPaths)
            )
        )
        # Pass the function to execute
        self.videoThumbnailGeneratorWorker = FileMetadataWorker(
            videoPaths,
            self.on_generate_video_thumbnails_execute_thread,
            desired_frame_indicies,
            desired_thumbnail_sizes,
        )  # Any other args, kwargs are passed to the run function
        self.videoThumbnailGeneratorWorker.signals.result.connect(
            self.on_generate_video_thumbnails_print_output
        )
        self.videoThumbnailGeneratorWorker.signals.finished.connect(
            self.on_generate_video_thumbnails_thread_complete
        )
        self.videoThumbnailGeneratorWorker.signals.progress.connect(
            self.on_generate_video_thumbnails_progress_fn
        )
    ...
    def on_generate_video_thumbnails_execute_thread(
        self,
        active_video_paths,
        desired_frame_indicies,
        desired_thumbnail_sizes,
        ...
        self.pending_operation_status.restart(
            OperationTypes.FilesystemThumbnailGeneration,
            numFilesToGenerateThumbnailsFor,
        )
        ...
        for (sub_index, aFoundVideoFile) in enumerate(active_video_paths):
            ...
            if aFoundVideoFile not in self.cache.keys():
                # Create the cache.
                self.cache[aFoundVideoFile] = VideoSpecificThumbnailCache(
                    aFoundVideoFile, parent=self
                )
                self.cache[aFoundVideoFile].frame_thumbnails_updated.connect(
                    self.on_cache_frame_thumbnails_updated
                )
            ...
            if desired_frame_indicies is not None:
                ...
                result_tuple = self.cache[aFoundVideoFile].reserve_frame_results(
                    desired_frame_indicies
                )
                already_loaded_frames = result_tuple[0]
                new_desired_frame_indicies = result_tuple[1]

                curr_frames_result_dict = self.cache[
                    aFoundVideoFile
                ].get_frames_results_dict()
                ...
                for anAlreadyLoadedFrame in already_loaded_frames:
                    videoThumbnailResultObj = curr_frames_result_dict[
                        anAlreadyLoadedFrame
                    ]
                    if videoThumbnailResultObj is not None:
                        self.videoFrameThumbnailsUpdated.emit(
                            aFoundVideoFile, videoThumbnailResultObj
                        )

            # Iterate through all found video-files in a given list (a list of VideoThumbnail objects: [VideoThumbnail])
            generatedThumbnailObjsList = self.generate_thumbnails_for_video_file(
                aFoundVideoFile,
                new_desired_frame_indicies,
                desired_thumbnail_sizes,
                enable_debug_print=False,
            )
            self.cache[aFoundVideoFile].update_frame_thumbnail_results(
                generatedThumbnailObjsList
            )
            ...
            self.videoThumbnailGenerationComplete.emit(
                aFoundVideoFile, generatedThumbnailObjsList
            )

            parsedFiles = parsedFiles + 1
            progress_callback.emit(
                active_video_paths,
                (parsedFiles * 100 / numFilesToGenerateThumbnailsFor),
            )
    ...
    def generate_thumbnails_for_video_file(
        self,
        activeVideoFilePath,
        desired_frame_indicies,
        thumbnailSizes,
        ...
        if enable_debug_print:
            print(
                "generate_thumbnails_for_video_file({0})...".format(
                    str(activeVideoFilePath)
                )
            )

        outputThumbnailObjsList = []
        frames = VideoPreviewThumbnailGenerator.video_to_desired_frames(
            activeVideoFilePath, desired_frame_indicies, enable_debug_print
        )

        # Generate and save thumbs
        for i in range(len(frames)):
            thumbs = VideoPreviewThumbnailGenerator.image_to_thumbs(
                frames[i], thumbnailSizes, enable_debug_print
            )
    ...
    @staticmethod
    def video_to_desired_frames(
        video_filename,
        desired_frame_indexes,
        enable_debug_print,
            ...
            if cap.isOpened() and video_length > 0:
                ...
                for aFrameNumber in frame_ids:
                    ...
                    if success:
                        if enable_debug_print:
                            print(
                                "frame_number[{0}]: SUCCESS!".format(str(aFrameNumber))
                            )
    ...
    def video_to_frames(video_filename, enable_debug_print, use_OpenCV_method=True):
        """Extract frames from video"""
        return VideoPreviewThumbnailGenerator.video_to_desired_frames(
            video_filename, None, enable_debug_print, use_OpenCV_method
        )
