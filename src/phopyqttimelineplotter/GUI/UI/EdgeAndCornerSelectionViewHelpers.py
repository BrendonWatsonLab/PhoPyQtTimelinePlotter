...
from PyQt5.QtCore import Q_FLAGS, QMargins, QPoint, QRect, QSize, Qt, pyqtSignal
from PyQt5.QtGui import QBrush, QColor, QCursor, QPainterPath
...
class EdgeAndCornerContainerViewMixin(object):
    ...
    def updateEdgeAndCornerContainerActivePosition(
        self, e_pos: QPoint, is_updating_hover
    ):
        didModeChange = False
        ...
        if (
            (
                (e_pos.y() > self.y() + self.height() - self.border_handle_size_diff)
                and (e_pos.x() < self.x() + self.border_handle_size_diff)  # Bottom
            )
            or  # Left
            # Right-Bottom
            (
                (e_pos.y() > self.y() + self.height() - self.border_handle_size_diff)
                and (  # Bottom
                    e_pos.x() > self.x() + self.width() - self.border_handle_size_diff
                )
            )
            or  # Right
            # Left-Top
            (
                (e_pos.y() < self.y() + self.border_handle_size_diff)
                and (e_pos.x() < self.x() + self.border_handle_size_diff)  # Top
            )
            or  # Left
            # Right-Top
            (e_pos.y() < self.y() + self.border_handle_size_diff)
            ...
            if (
                ...
                e_pos.x() < self.x() + self.border_handle_size_diff
            ):  # Left
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Corner_BL, is_updating_hover
                )
                # Right - Bottom
            if (
                e_pos.y() > self.y() + self.height() - self.border_handle_size_diff
                ...
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Corner_BR, is_updating_hover
                )
            # Left - Top
            if (e_pos.y() < self.y() + self.border_handle_size_diff) and (  # Top
                ...
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Corner_TL, is_updating_hover
                )
            # Right - Top
            if (e_pos.y() < self.y() + self.border_handle_size_diff) and (  # Top
                e_pos.x() > self.x() + self.width() - self.border_handle_size_diff
                ...
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Corner_TR, is_updating_hover
                )
        ...
        if (
            (not didModeChange)
            and (e_pos.x() < self.x() + self.border_handle_size_diff)
            or (  # Left
                e_pos.x() > self.x() + self.width() - self.border_handle_size_diff
            )
        ):  # Right
            if e_pos.x() < self.x() + self.border_handle_size_diff:  # Left
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Edge_Left, is_updating_hover
                )
            else:  # Right
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Edge_Right, is_updating_hover
                )

        # check cursor vertical position (check top and bottom edges)
        if (
            (not didModeChange)
            and (e_pos.y() > self.y() + self.height() - self.border_handle_size_diff)
            or (e_pos.y() < self.y() + self.border_handle_size_diff)  # Bottom
        ):  # Top
            if e_pos.y() < self.y() + self.border_handle_size_diff:  # Top
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Edge_Top, is_updating_hover
                )
            else:  # Bottom
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.Edge_Bottom, is_updating_hover
                )

        # Otherwise we're not hovering any corners or edges. Check and see if we're moving
        if not didModeChange:
            didModeChange = self.try_add_component(
                EdgeAndCornerContainerComponent.BODY, is_updating_hover
            )
            if not didModeChange:
                # if we don't allow MOVE mode, set to NONE mode
                didModeChange = self.try_add_component(
                    EdgeAndCornerContainerComponent.NONE, is_updating_hover
                )
    ...
    def paintEvent_EdgeAndCornerContainerViewMixin(self, painter, drawRect):
        ...
        currHighlightRect = self.hoveredEdgeAndCorners.get_highlight_rect(
            drawRect, self.border_handle_size_diff
        )
