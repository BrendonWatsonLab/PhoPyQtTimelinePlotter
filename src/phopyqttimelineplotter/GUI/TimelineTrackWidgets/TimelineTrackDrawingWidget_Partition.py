...
#
class TimelineTrackDrawingWidget_Partition(
    TrackConfigDataCacheMixin, TrackConfigMixin, TimelineTrackDrawingWidgetBase
):
    ...
    def __init__(
        ...
        self.shouldDismissSelectionUponMouseButtonRelease = (
            TimelineTrackDrawingWidget_Partition.default_shouldDismissSelectionUponMouseButtonRelease
        )
        self.itemSelectionMode = (
            TimelineTrackDrawingWidget_Partition.default_itemSelectionMode
        )
    ...
    def reloadModelFromConfigCache(self):
        ...
        if self.partitionManager is None:
            self.partitionManager = Partitioner(
                self.totalStartTime,
                self.totalEndTime,
                self,
                ...
                name="name",
                partitionViews=None,
                partitionDataObjects=self.durationRecords,
            )
        else:
    ...
    def on_partition_modify_event(self, childIndex):
        ...
        if not (selectedPartitionViewObject is None):
            self.activePartitionEditDialog = PartitionEditDialog(
                self.database_connection, parent=self
            )
            self.activePartitionEditDialog.set_referred_object_identifiers(
                self.get_trackID(), selectedPartitionIndex
            )
            self.activePartitionEditDialog.set_start_date(
                selectedPartitionViewObject.startTime
            )
            self.activePartitionEditDialog.set_end_date(
                selectedPartitionViewObject.endTime
            )
            self.activePartitionEditDialog.set_type(selectedPartitionViewObject.type_id)
            self.activePartitionEditDialog.set_subtype(
                selectedPartitionViewObject.subtype_id
            )
            self.activePartitionEditDialog.set_title(selectedPartitionViewObject.name)
            self.activePartitionEditDialog.set_subtitle(
                selectedPartitionViewObject.subtitle
            )
            ...
            self.activePartitionEditDialog.set_id_values(
                sel_behavioral_box_id, sel_experiment_id, sel_cohort_id, sel_animal_id
            )
            self.activePartitionEditDialog.set_id_frame_editability(
                False
            )  # Ensure the id options aren't editable. For partitions they're determined entirely by the track's config.

            self.activePartitionEditDialog.on_commit.connect(self.try_update_partition)
            self.activePartitionEditDialog.on_cancel.connect(
                self.partition_dialog_canceled
            )
        else:
    ...
    def paintEvent(self, event):
        ...
        for (index, obj) in enumerate(self.partitions):
            obj.get_view().update()
            self.eventRect[index] = obj.get_view().paint(
    ...
    def set_active_filter(self, start_datetime, end_datetime):
        # Draw the duration objects
        for (index, obj) in enumerate(self.partitions):
            obj.get_view().is_deemphasized = not obj.overlaps_range(
    ...
    def on_button_clicked(self, event):
        ...
        if newlySelectedObjectIndex is None:
            ...
            # Select the object
            if self.selected_partition_object_indicies.__contains__(
                ...
                if self.itemSelectionMode is ItemSelectionOptions.SingleSelection:
                    if len(self.selected_partition_object_indicies) > 0:
                        # Deselect previously selected item
                        prevSelectedItemIndex = self.selected_partition_object_indicies[
                            0
                        ]
                        self.selected_partition_object_indicies.remove(
                            prevSelectedItemIndex
                        )
                        self.partitions[
                            prevSelectedItemIndex
                        ].get_view().on_button_released(event)
                ...
                self.partitions[newlySelectedObjectIndex].get_view().on_button_clicked(
                    event
                )
                self.update()
                self.selection_changed.emit(self.trackID, newlySelectedObjectIndex)
    ...
    def try_update_partition(
        ...
        if (dialog_child_partition_index is not None) and (
            0 <= dialog_child_partition_index <= (len(self.partitions) - 1)
        ):
            ...
            print(
                "Modifying partition[{0}]: (type_id: {1}, subtype_id: {2})".format(
                    dialog_child_partition_index, type_id, subtype_id
                )
            )
            ...
            self.partitionManager.modify_partition(
                dialog_child_partition_index,
                start_date,
                end_date,
                title,
                subtitle,
            ...
            print(
                "Modified partition[{0}]: (type_id: {1}, subtype_id: {2})".format(
                    dialog_child_partition_index, type_id, subtype_id
                )
            )
            self.reinitialize_from_partition_manager()
            ...
            print(
                "Error: try_update_partition(...): dialog_child_partition_index {0} is not valid".format(
                    str(dialog_child_partition_index)
                )
            )
