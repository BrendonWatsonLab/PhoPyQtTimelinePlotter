...
from phopyqttimelineplotter.GUI.TimelineTrackWidgets.TimelineTrackDrawingWidget_EventsBase import (
    TimelineTrackDrawingWidget_EventsBase,
)
...
class TimelineTrackDrawingWidget_Videos(
    TrackConfigDataCacheMixin, TrackConfigMixin, TimelineTrackDrawingWidget_EventsBase
):
    ...
    def __init__(
        ...
        super(TimelineTrackDrawingWidget_Videos, self).__init__(
        ...
        self.child_action_generate_thumbnails.connect(
            self.parent().on_video_track_child_generate_thumbnails
        )
    ...
    def attach_child_duration_object_signals(self):
        ...
        for aDurationObject in self.durationObjects:
            aDurationObject.on_generate_thumbnails.connect(
                self.on_child_action_generate_thumbnails
            )
    ...
    def mouseDoubleClickEvent(self, event):
        print(
            "TimelineTrackDrawingWidget_Videos.mouseDoubleClickEvent(): Mouse double clicked! ({0},{1})".format(
                event.x(), event.y()
            )
        )
    ...
    def on_video_modify_event(self):
        ...
        if selectedVideoObject:
            ...
            self.activeVideoEditDialog = VideoEditDialog(
            ...
            self.activeVideoEditDialog.set_subtitle(
                str(selectedVideoObject.get_parent_url())
            )
            self.activeVideoEditDialog.set_body(str(selectedVideoObject.extended_data))
            print(selectedVideoObject.extended_data)

            (
                sel_behavioral_box_id,
                sel_experiment_id,
                sel_cohort_id,
                sel_animal_id,
            ) = selectedVideoObject.get_ids()
            self.activeVideoEditDialog.set_id_values(
                sel_behavioral_box_id, sel_experiment_id, sel_cohort_id, sel_animal_id
            )
            self.activeVideoEditDialog.set_is_original_video(
                selectedVideoObject.get_is_original_video()
            )
    ...
    def on_create_playhead_selection(self, trackID, desired_datetime):
        print(
            "TimelineTrackDrawingWidget_Videos.on_create_playhead_selection(trackID: {0}, desired_datetime: {1})".format(
                str(trackID), str(desired_datetime)
            )
        )
    ...
    def on_child_action_generate_thumbnails(self, childIndex):
        print(
            "TimelineTrackDrawingWidget_Videos.on_child_action_generate_thumbnails({0})".format(
                str(childIndex)
            )
        )
