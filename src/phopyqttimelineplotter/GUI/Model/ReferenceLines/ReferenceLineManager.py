...
from phopyqttimelineplotter.GUI.Model.ReferenceLines.ReferenceMarker import (
    ReferenceMarkerManagerConfiguration,
    RepresentedMarkerRecord,
    RepresentedMarkerTime,
    RepresentedTimeRange,
)
...
class ReferenceMarkerManager(IndicatorLineMixin, DurationRepresentationMixin, QObject):
    ...
    def __init__(
        self, totalStartTime, totalEndTime, drawWidth, num_markers, parent=None
    ):
    ...
    def minor_markers_daterange(start_date, end_date, hoursBetween):
        # TODO: Zero seconds
        hours_only_start_date = datetime(
        ...
        numSecondsBetween = int(
            ReferenceMarkerManager.secondsBetween(start_date, end_date)
        )
    ...
    def bulk_add_static_marker_data(self):
        ...
        for single_date in ReferenceMarkerManager.minor_markers_daterange(
            start_date, end_date, self.minorMarkersHoursSpacing
        ):
            newObj = RepresentedMarkerTime(single_date, self)
            # print(newObj.time_string)
    ...
    def add_special_indicator_markers(self):
        ...
        new_view_obj = ReferenceMarker(
            "video_playback", False, properties=curr_properties, parent=self
        )
        ...
        self.videoPlaybackIndicatorContainer = ModelViewContainer(
            new_record_obj, new_view_obj
        )

        # Hover line
        curr_properties = ReferenceMarkerManager.hoverLineProperties
        new_view_obj = ReferenceMarker(
            "hover", False, properties=curr_properties, parent=self
        )
        new_view_obj.update_position(0.0, self.get_scale())
        new_view_obj.drawsPointer = False
        new_record_obj = RepresentedMarkerRecord(datetime.now(), parent=self)
        self.userMouseHoverIndicatorContainer = ModelViewContainer(
            new_record_obj, new_view_obj
        )
        ...
        new_view_obj = ReferenceMarker(
            "select", False, properties=curr_properties, parent=self
        )
        new_view_obj.update_position(0.0, self.get_scale())
        new_view_obj.drawsPointer = False
        new_record_obj = RepresentedMarkerRecord(datetime.now(), parent=self)
        self.userMouseSelectionIndicatorContainer = ModelViewContainer(
            new_record_obj, new_view_obj
        )
    ...
    def bulk_add_reference_makers(self, num_markers):
        ...
        for a_marker_index in range(num_markers):
            curr_color = QColor.fromHslF(
                (float(a_marker_index) / float(num_markers)), 0.9, 0.9, 1.0
            )
            curr_properties = TickProperties(curr_color, 0.9, Qt.SolidLine)
            new_view_obj = ReferenceMarker(
                str(a_marker_index), False, properties=curr_properties, parent=self
            )
            new_view_obj.update_position(0.0, self.get_scale())
    ...
    def update_next_unused_marker(self, new_datetime, currDrawWidth):
        ...
        if potential_unused_marker_key is None:
            ...
            self.get_markers()[
                potential_unused_marker_key
            ].get_record().time = new_datetime

            # Get the x_offset position from the datetime provided
            # item_x_offset = self.compute_x_offset_from_datetime(new_datetime)

            self.get_markers()[potential_unused_marker_key].get_view().update_position(
                0.0, self.get_scale()
            )
    ...
    def compute_x_offset_from_datetime(self, drawWidth, aDatetime):
        ...
        item_x_offset = ReferenceMarkerManager.percent_offset_to_track_offset(
            drawWidth, item_percent_offset
        )
    ...
    def draw(self, painter, event, scale):
        ...
        for (id_key, value) in self.markersDict.items():
            ...
            if self.needs_positions_update:
                ...
                item_x_offset = self.compute_x_offset_from_datetime(
                    drawWidth, itemDatetime
                )
    ...
    def update_marker_metadata(self, marker_metadata_list):
        ...
        self.used_markers_extended_data_updated.emit(
            self.used_mark_extended_metadata_stack
        )
    ...
    def show_active_markers_list(self):
        ...
        self.used_markers_updated.connect(
            self.activeMarkersWindow.on_active_markers_list_updated
        )
        self.used_markers_extended_data_updated.connect(
            self.activeMarkersWindow.on_active_markers_metadata_updated
        )
