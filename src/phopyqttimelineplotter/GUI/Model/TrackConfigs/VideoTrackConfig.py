...
class VideoTrackFilter(TrackFilterBase):
    def __init__(
        ...
        super(VideoTrackFilter, self).__init__(
            VideoFile,
            behavioral_box_ids,
            experiment_ids,
            cohort_ids,
    ...
    def build_filter_query(self, session):
        ...
        if self.allow_original_videos and self.allow_labeled_videos:
            print(
                "Warning: both allow_original_videos and allow_labeled_videos are True!"
            )
            ...
            # both are false
            print(
                "Warning: both allow_original_videos and allow_labeled_videos are False!"
            )
            query = query.filter(self.trackRecordClass.is_original_video == None)
        return query

    def __str__(self):
        return "VideoTrackFilter: behavioral_box_ids: {0}, experiment_ids: {1}, cohort_ids: {2}, animal_ids: {3}, allow_original_videos: {4}, allow_labeled_videos: {5}".format(
            self._get_behavioral_box_ids_str(),
            self._get_experiment_ids_str(),
            self._get_cohort_ids_str(),
            self._get_animal_ids_str(),
            self.allow_original_videos,
            self.allow_labeled_videos,
        )
    ...
    def get_output_dict(self):
        return {
            ...
            "allow_original_videos": self.allow_original_videos,
            "allow_labeled_videos": self.allow_labeled_videos,
        }
...
class VideoTrackConfiguration(TrackConfigurationBase):
    def __init__(
        ...
        super(VideoTrackConfiguration, self).__init__(
            trackIndex,
            trackTitle,
            trackExtendedDescription,
            VideoFile,
    ...
    def __str__(self):
        return "VideoTrackConfiguration: trackIndex: {0}, trackTitle: {1}, trackExtendedDescription: {2}, filter: {3}".format(
            self.trackIndex,
            self.trackTitle,
