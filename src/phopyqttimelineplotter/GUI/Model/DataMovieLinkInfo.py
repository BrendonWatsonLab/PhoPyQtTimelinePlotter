...
class DataMovieLinkInfo(SimpleErrorStatusMixin, QObject):
    ...
    video_playback_position_changed = pyqtSignal(
        float
    )  # signal emitted when the video window changes the playback position. Sent to main timeline window

    timeline_datetime_position_changed = pyqtSignal(
        float
    )  # signal emitted when the timeline changes the playhead position. Sent to video player window.
    ...
    def __init__(
        self,
        videoEventChildReference,
        videoPlayerWindowRef,
        mainTimelineWindowRef,
        parent=None,
        sync_option=DataMovieLink_SyncOption.Bidirectional,
    ):
        ...
        self.videoPlayerWindow.video_playback_position_updated.connect(
            self.update_video_playback_position
        )
        ...
        self.video_playback_position_changed.connect(
            self.mainTimelineWindowRef.on_video_playback_position_updated
        )

        ## TODO NOW: Comment this out if we don't want it.
        # Bound to trigger the video player to update on timeline adjust
        self.timeline_datetime_position_changed.connect(
            self.videoPlayerWindow.on_timeline_position_updated
        )
        ...
        if self._shouldGenerateVideoThumbnails == True:
            ...
            self.videoThumbnailGenerator.videoFrameThumbnailsUpdated.connect(
                self.on_cache_frame_thumbnails_updated
            )  # Single frame of a signle video file
            # Connect signal to the video player window
            self.thumbnail_generated.connect(
                self.videoPlayerWindow.on_video_thumbnail_generated
            )
    ...
    def generate_thumbnails(self, desired_frame_indicies):
        proposed_video_file_path = self.get_video_url()
        if proposed_video_file_path is not None:
            self.get_video_thumbnail_generator().reload_data(
                [str(proposed_video_file_path)], desired_frame_indicies
            )
        else:
    ...
    def compute_absolute_time(self, video_duration_percent):
        video_duration_relative_offset = (
            video_duration_percent * self.get_video_duration()
        )
        return self.get_video_duration_event_obj().compute_absolute_time(
            video_duration_relative_offset
        )
    ...
    def compute_video_percent_offset(self, absolute_time):
        ...
        if not self.get_video_duration_event_obj().contains(absolute_time):
            ...
            video_duration_relative_offset = (
                self.get_video_duration_event_obj().compute_relative_offset_duration(
                    absolute_time
                )
            )
    ...
    def compute_timeline_percent_offset(self, absolute_time):
        ...
        if not self.mainTimelineWindowRef.contains_date(absolute_time):
            ...
            timeline_duration_relative_offset = (
                self.mainTimelineWindowRef.compute_relative_offset_duration(
                    absolute_time
                )
            )
    ...
    @pyqtSlot(float)
    def update_video_playback_position(
        self, updatedRelativePercentVideoPlaybackPosition
    ):
        self.active_relative_video_playback_percent_offset = (
            updatedRelativePercentVideoPlaybackPosition
        )
        self.active_absolute_datetime = self.compute_absolute_time(
            updatedRelativePercentVideoPlaybackPosition
        )
        self.active_relative_timeline_percent_offset = (
            self.compute_timeline_percent_offset(self.active_absolute_datetime)
        )
        # self.video_playback_position_changed.emit(absolute_datetime)
        self.video_playback_position_changed.emit(
            self.active_relative_timeline_percent_offset
        )
    ...
    def update_timeline_playhead_position(self, updatedDatetime):
        ...
        self.active_relative_video_playback_percent_offset = (
            self.compute_video_percent_offset(self.active_absolute_datetime)
        )
        self.timeline_datetime_position_changed.emit(
            self.active_relative_video_playback_percent_offset
        )
    ...
    def on_cache_frame_thumbnails_updated(self, videoFileName, videoThumbnailResultObj):
        if videoFileName != str(self.get_video_url()):
            ...
            print(
                "WARNING: name doesn't match! (updated result video filename: {0}, target video file name: {1}".format(
                    str(videoFileName), str(self.get_video_url())
                )
            )
            return
    ...
    def on_video_event_thumbnail_generation_complete(
        ...
        print(
            "DataMovieLinkInfo.on_video_event_thumbnail_generation_complete(videoFileName: {0})...".format(
                str(videoFileName)
            )
        )
