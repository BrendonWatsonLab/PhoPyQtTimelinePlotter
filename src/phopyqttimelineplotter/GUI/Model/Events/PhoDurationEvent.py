...
from phopyqttimelineplotter.GUI.Helpers.ThumbnailDrawingEventMixin import (
    ThumbnailDrawingEventMixin,
)
from phopyqttimelineplotter.GUI.Model.Events.PhoEvent import *
from phopyqttimelineplotter.GUI.UI.EdgeAndCornerSelectionViewHelpers import (
    EdgeAndCornerContainerComponent,
    EdgeAndCornerContainerViewMixin,
)


class PhoDurationEvent(
    ThumbnailDrawingEventMixin, EdgeAndCornerContainerViewMixin, PhoEvent
):
    ...
    def __init__(
        ...
        endTime=None,
        name="",
        color=QColor(51, 204, 255, PhoEvent.DefaultOpacity),
        extended_data=dict(),
        parent=None,
    ):
        super(PhoDurationEvent, self).__init__(
            startTime, name, color, extended_data, parent=parent
        )
    ...
    def __eq__(self, otherEvent):
        return (
            ...
            and self.startTime == otherEvent.startTime
            and self.endTime == otherEvent.endTime
        )
    ...
    def is_entirely_less_than(self, otherEvent):
        ...
        return (
            self.startTime < otherEvent.startTime
            and self.endTime < otherEvent.startTime
        )
    ...
    def overlaps_range(self, range_start_datetime, range_end_datetime):
        ...
        if self.endTime:
            ...
            if (self.startTime >= range_start_datetime) and (
                self.startTime <= range_end_datetime
            ):
                return True
            else:
                return False
    ...
    def contains(self, absolute_datetime):
        if self.is_instantaneous_event():
            ...
            return (self.startTime <= absolute_datetime) and (
                self.endTime >= absolute_datetime
            )
    ...
    def __str__(self):
        return "Event {0}: [startTime: {1}, endTime: {2}], duration: {3}, extended_data: {4}".format(
            self.name,
            self.startTime,
            self.endTime,
            self.computeDuration(),
            str(self.extended_data),
        )
    ...
    def handleMenuAction(self, action):
        if action == self.info_action:
            ...
            print(
                "Unknown menu option! for track {0}".format(str(self.get_track_index()))
            )
            return action
    ...
    def compute_parent_offset_rect(
        self,
        totalStartTime,
        totalEndTime,
        totalDuration,
    ...
    def set_painter_config(self, aPainter):
        ...
        if self.is_active:
            currPenWidth = 4.0
            currPenColor = (
                PhoDurationEvent.ColorBorderActive
            )  # For active events, override the pen color too
            currFillColor = (
                PhoDurationEvent.ColorActive
            )  # For active events, override the color with the current active color
    ...
    def paint(
        ...
        if self.is_instantaneous_event():
            ...
            painter.drawRoundedRect(
                ...
                width,
                height,
                PhoDurationEvent.RectCornerRounding,
                PhoDurationEvent.RectCornerRounding,
            )
