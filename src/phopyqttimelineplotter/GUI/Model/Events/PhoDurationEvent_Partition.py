...
class PhoDurationEvent_Partition(PhoDurationEvent):
    ...
    def __init__(
        ...
        subtitle="",
        body="",
        color=QColor(55, 55, 55, PhoEvent.DefaultOpacity),
        type_id=None,
        subtype_id=None,
        extended_data=dict(),
        parent=None,
    ):
        super(PhoDurationEvent_Partition, self).__init__(
    ...
    def on_key_pressed(self, event):
        ...
        if gey == Qt.Key_M:
            ...
            print(
                "PhoDurationEvent_Partition.on_key_pressed: Right key pressed!, call drawFundBlock()"
            )
    ...
    def set_painter_config(self, aPainter):
        ...
        if self.is_deemphasized:
            currFillColor = self.color.darker(
                300
            )  # setting to 300 returns a color that has one-third the brightness
        else:
            # de-emphasized overrides emphasized status
            if self.is_emphasized:
                currFillColor = self.color.lighter(
                    110
                )  # returns a color that's 20% brighter
            else:
                currFillColor = self.color
    ...
    def paint(
        ...
        parent_modified_event_rect = super(PhoDurationEvent_Partition, self).paint(
            painter, totalStartTime, totalEndTime, totalDuration, totalParentCanvasRect
        )
