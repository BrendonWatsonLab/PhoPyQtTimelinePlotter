...
class ExampleDatabaseTableWindow(AbstractDatabaseAccessingWindow):
    ...
    def __init__(self, database_connection):
        ...
        for (i, tableRecordClass) in enumerate(
            ExampleDatabaseTableWindow.ActiveTableTabs
        ):
    ...
    def delete_record(self):
        ...
        index_list = []
        for model_index in (
            self.tables[curr_active_tab_index].selectionModel().selectedRows()
        ):
        ...
        if reply == QMessageBox.Yes:
            for index in index_list:
                self.current_record = self.models[curr_active_tab_index].record(
                    index.row()
                )
    ...
    def store_new_record(self, name):
        self.currClass = ExampleDatabaseTableWindow.ActiveTableTabs[
            self.activeActionTabIndex
        ]
        self.currClassString = ExampleDatabaseTableWindow.ActiveTableTabStrings[
            self.activeActionTabIndex
        ]
    ...
    def export_records(self):
        ...
        for irow in range(currModel.rowCount(None)):
            ...
            for icol in range(currModel.columnCount(None)):
                curr_cell = currModel.data(
                    currModel.createIndex(irow, icol), Qt.DisplayRole
                )
