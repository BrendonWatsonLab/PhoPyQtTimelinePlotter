...
class GeneralDataFilesystemLoader(QObject):
    ...
    def __init__(self, generalFilePaths, parent=None):
        super(GeneralDataFilesystemLoader, self).__init__(parent=parent) # Call the inherited classes __init__ method
        self.cache = dict()
    ...
    def add_labjack_file_path(self, newLabjackFilePath):
        if (newLabjackFilePath in self.generalFilePaths):
            print("WARNING: {0} is already in generalFilePaths! Not adding again.".format(str(newLabjackFilePath)))
            return False
    ...
    @pyqtSlot(list, int)
    def on_load_general_data_files_progress_fn(self, latest_labjack_file_result_list, n):
        aFoundGeneralDataFile = latest_labjack_file_result_list[0] # The last loaded URL
        outEventFileObj = latest_labjack_file_result_list[1] # outEventFileObj: LabjackEventFile type object
        print('on_load_general_data_files_progress_fn(..., n: {}): file: {}'.format(str(n), str(aFoundGeneralDataFile)))
        ...
        if (not (aFoundGeneralDataFile in self.cache.keys())):
            ...
            print("WARNING: labjack file path {} already exists in the cache. Updating its values...".format(str(aFoundGeneralDataFile)))
            self.cache[aFoundGeneralDataFile] = outEventFileObj
            pass
    ...
    """
    def on_load_general_data_files_execute_thread(self, active_general_data_file_paths, progress_callback):
        ...
        for (sub_index, aFoundGeneralDataFile) in enumerate(active_general_data_file_paths):
            ...
            (dateTimes, labjackEventContainers, phoServerFormatArgs) = GeneralDataFilesystemLoader.loadLabjackEventsFile(aFoundGeneralDataFile, self.videoStartDates, self.videoEndDates, shouldLimitEventsToVideoDates=False, usePhoServerFormat=True, phoServerFormatIsStdOut=False, should_filter_for_invalid_events=should_filter_for_invalid_events)
            ...
            if (not (aFoundGeneralDataFile in active_cache.keys())):
                ...
                # Parent already exists
                print("WARNING: labjack file path {} already exists in the temporary cache. Updating its values...".format(str(aFoundGeneralDataFile)))
                active_cache[aFoundGeneralDataFile] = outEventFileObj
                pass
            ...
            progress_callback.emit([aFoundGeneralDataFile, outEventFileObj], (parsedFiles*100/numPendingFiles))
    ...
    @pyqtSlot(list)
    def on_load_general_data_files_thread_complete(self, finished_loaded_general_data_files):
        print("THREAD on_load_general_data_files_thread_complete(...)! {0}".format(str(finished_loaded_general_data_files)))
    ...
    """
    @staticmethod
    def loadLabjackEventsFile(generalFilePath, videoDates, videoEndDates, shouldLimitEventsToVideoDates=True, limitedVariablesToCreateEventsFor=None, usePhoServerFormat=False, phoServerFormatIsStdOut=True, should_filter_for_invalid_events=True):
        ...
        (dateTimes, onesEventFormatDataArray, phoServerFormatArgs) = LabjackEventsLoader.loadLabjackEventsFile_loadFromFile(generalFilePath, usePhoServerFormat, phoServerFormatIsStdOut)
