...
helper_timenum_to_datetime = np.vectorize( lambda x: matlab2datetime( x ) )
...
def loadLabjackDataFromMatlabFormat(filePath):
    ...
    file = loadLabjackDataNew(filePath)
    ...
    dateTimes = np.array(helper_timenum_to_datetime(dateNums))
...
def loadAndConcatenateLabjackDataFromPhoServerFormat(filePaths):
    ...
    for (fileIndex, aFilePath) in enumerate(filePaths):
        (relevantFileLines, relevantDateTimes, dateTimes, onesEventFormatDataArray) = loadLabjackDataFromPhoServerFormat(aFilePath)
...
if __name__ == '__main__':
    print('TESTING')
    # (relevantFileLines, relevantDateTimes, dateTimes, onesEventFormatDataArray) = loadLabjackDataFromPhoServerFormat(r'G:\Google Drive\Modern Behavior Box\Results - Data\Behavioral Box Data\Labjack Data\Complete.txt', shouldUseStdOutFormat=True)
    # print(dateTimes, onesEventFormatDataArray)
    # targetPaths = [r'G:\Google Drive\Modern Behavior Box\Results - Data\Behavioral Box Data\Labjack Data\Complete.txt',
    #  r'G:\Google Drive\Modern Behavior Box\Results - Data\Behavioral Box Data\Labjack Data\Complete -07-05-19.txt',
    #  r'G:\Google Drive\Modern Behavior Box\Results - Data\Behavioral Box Data\Labjack Data\Complete -07-08-19.txt',
    #  r'G:\Google Drive\Modern Behavior Box\Results - Data\Behavioral Box Data\Labjack Data\Complete -07-10-19.txt']
    # combinedFileLines = loadAndConcatenateLabjackDataFromPhoServerFormat(targetPaths)
    # writeLinesToCsvFile(combinedFileLines)

    (relevantFileLines, relevantDateTimes, dateTimes, onesEventFormatDataArray) = loadLabjackDataFromPhoServerFormat(r'\\WATSON-BB-OVERS\ServerInternal-00\EventData\BB01\out_file_s470017560_46Combined.csv', shouldUseStdOutFormat=False)
    print(dateTimes, onesEventFormatDataArray)
