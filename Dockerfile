...
# and `pip install PhoPyQtTimelinePlotter` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "PhoPyQtTimelinePlotter", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n PhoPyQtTimelinePlotter -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "PhoPyQtTimelinePlotter", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "PhoPyQtTimelinePlotter", "YOUR_CONSOLE_SCRIPT"]
