...
class PhoDurationEvent_Video(PhoDurationEvent):
    ...
    def computeDuration(self):
        if self.endTime:
            ...
            return PhoDurationEvent_Video.InstantaneousEventDuration
    ...
    def set_painter_config(self, aPainter):
        ...
        currPenColor = PhoDurationEvent_Video.ColorBorderBase
        currPenWidth = 1.0
        ...
        if self.is_deemphasized:
            ...
            if self.is_emphasized:
                currFillColor = PhoDurationEvent_Video.ColorEmph
            else:
                currFillColor = self.color
        ...
        if self.is_active:
            currPenWidth = 4.0
            currPenColor = PhoDurationEvent_Video.ColorBorderActive # For active events, override the pen color too
            currFillColor = PhoDurationEvent_Video.ColorActive # For active events, override the color with the current active color
    ...
    def paint(self, painter, totalStartTime, totalEndTime, totalDuration, totalParentCanvasRect):
        ...
        #     # de-emphasized overrides emphasized status
        #     if self.is_emphasized:
        #         activeColor = PhoDurationEvent_Video.ColorEmph
        #     else:
        #         activeColor = self.color

        # if self.is_active:
        #     painter.setPen(QtGui.QPen(PhoDurationEvent_Video.ColorBorderActive, 4.0, join=Qt.MiterJoin))
        #     painter.setBrush(QBrush(PhoDurationEvent_Video.ColorActive, Qt.SolidPattern))
        # else:
        #     painter.setPen(QtGui.QPen(PhoDurationEvent_Video.ColorBorderBase, 1.5, join=Qt.MiterJoin))
        ...
        if self.is_instantaneous_event():
            ...
            painter.drawRoundedRect(x, y, width, height, PhoDurationEvent_Video.RectCornerRounding, PhoDurationEvent_Video.RectCornerRounding)
